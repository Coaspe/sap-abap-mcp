...
source patch/activate, text element
...
`sap.repository.delete.preview`는 cleanup 직전에 RUN_OWNED exact URI 한 개만
대상으로 호출한다. 이 preview가 반환한 planId/confirmation은 같은 객체의
`sap.repository.delete.execute`에만 사용한다.
...
사용한다. 정확한 도구 순서는 `sap.repository.delete.preview` 다음
`sap.repository.delete.execute`이며, generic `sap.refactor.preview/execute`로
객체를 삭제하지 않는다. wildcard/package-wide delete를 금지한다.
