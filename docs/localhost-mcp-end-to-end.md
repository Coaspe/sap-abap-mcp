...
command = "npx.cmd"
args = ["-y", "@coaspe/sap-abap-mcp@0.3.0", "serve"]
```
...
`npx.cmd`가 패키지를 찾을 수 있도록 npm registry에 접근할 수 있어야 한다. 운영에서는 버전을 `@0.3.0`처럼 고정한다.
...
| 실제 ADT 로그인 검증 | 구현 |
| 연결 진단 `doctor` | 구현 |
| 실제 MCP 도구 | ABAP FS 2.6.5 기준 42개 + 확장 기능 8개, 총 50개 구현 및 인메모리 MCP 통합 테스트 완료 |
...
ABAP FS 호환 42개와 확장 8개 전체 이름은 [`src/compat/abap-fs-tools.ts`](../src/compat/abap-fs-tools.ts)에 고정되어 있다. 확장 범위는 semantic service, refactoring/delete, transport lifecycle, abapGit, RAP/service binding, inactive/version restore, transaction, cross-system compare, dependency graph다.
...
```powershell
npm view @coaspe/sap-abap-mcp@0.3.0 version
npx.cmd -y @coaspe/sap-abap-mcp@0.3.0 help
```
...
npx.cmd -y @coaspe/sap-abap-mcp@0.3.0 profile add QAS200 `
...
npx.cmd -y @coaspe/sap-abap-mcp@0.3.0 profile add PRD100 `
...
```powershell
npx.cmd -y @coaspe/sap-abap-mcp@0.3.0 profile list
```
...
npx.cmd -y @coaspe/sap-abap-mcp@0.3.0 auth login QAS200
npx.cmd -y @coaspe/sap-abap-mcp@0.3.0 auth login PRD100
```
...
```powershell
npx.cmd -y @coaspe/sap-abap-mcp@0.3.0 auth status DEV100
npx.cmd -y @coaspe/sap-abap-mcp@0.3.0 auth status QAS200
npx.cmd -y @coaspe/sap-abap-mcp@0.3.0 auth status PRD100
```
...
npx.cmd -y @coaspe/sap-abap-mcp@0.3.0 doctor QAS200
npx.cmd -y @coaspe/sap-abap-mcp@0.3.0 doctor PRD100
```
...
```powershell
claude mcp add --transport stdio --scope user sap-abap -- npx.cmd -y @coaspe/sap-abap-mcp@0.3.0 serve
```
...
```powershell
npx.cmd -y @coaspe/sap-abap-mcp@0.3.0 auth logout DEV100
```

프로필과 자격증명 삭제:

```powershell
npx.cmd -y @coaspe/sap-abap-mcp@0.3.0 profile remove DEV100
```
...
```powershell
npm install -g @coaspe/sap-abap-mcp@0.3.0
```
...
```powershell
npx.cmd -y @coaspe/sap-abap-mcp@0.3.0 profile remove DEV100
npx.cmd -y @coaspe/sap-abap-mcp@0.3.0 profile remove QAS200
npx.cmd -y @coaspe/sap-abap-mcp@0.3.0 profile remove PRD100
```
