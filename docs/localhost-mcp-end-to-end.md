...
command = "npx.cmd"
args = ["-y", "@coaspe/sap-abap-mcp@0.1.1", "serve"]
```
...
`npx.cmd`가 패키지를 찾을 수 있도록 npm registry에 접근할 수 있어야 한다. 운영에서는 버전을 `@0.1.1`처럼 고정한다.
...
2. 패키지를 빌드하고 테스트한다.
3. 버전을 고정해 public preview package로 게시한다.
...
```powershell
npm view @coaspe/sap-abap-mcp@0.1.1 version
npx.cmd -y @coaspe/sap-abap-mcp@0.1.1 help
```
...
```powershell
npx.cmd -y @coaspe/sap-abap-mcp@0.1.1 profile add DEV100 `
...
npx.cmd -y @coaspe/sap-abap-mcp@0.1.1 profile add QAS200 `
...
npx.cmd -y @coaspe/sap-abap-mcp@0.1.1 profile add PRD100 `
...
```powershell
npx.cmd -y @coaspe/sap-abap-mcp@0.1.1 profile list
```
...
```powershell
npx.cmd -y @coaspe/sap-abap-mcp@0.1.1 auth login DEV100
npx.cmd -y @coaspe/sap-abap-mcp@0.1.1 auth login QAS200
npx.cmd -y @coaspe/sap-abap-mcp@0.1.1 auth login PRD100
```
...
```powershell
npx.cmd -y @coaspe/sap-abap-mcp@0.1.1 auth status DEV100
npx.cmd -y @coaspe/sap-abap-mcp@0.1.1 auth status QAS200
npx.cmd -y @coaspe/sap-abap-mcp@0.1.1 auth status PRD100
```
...
```powershell
npx.cmd -y @coaspe/sap-abap-mcp@0.1.1 doctor DEV100
npx.cmd -y @coaspe/sap-abap-mcp@0.1.1 doctor QAS200
npx.cmd -y @coaspe/sap-abap-mcp@0.1.1 doctor PRD100
```
...
```powershell
codex mcp add sap-abap -- npx.cmd -y @coaspe/sap-abap-mcp@0.1.1 serve
```
...
```powershell
claude mcp add --transport stdio --scope user sap-abap -- npx.cmd -y @coaspe/sap-abap-mcp@0.1.1 serve
```
...
```powershell
npx.cmd -y @coaspe/sap-abap-mcp@0.1.1 auth logout DEV100
```

프로필과 자격증명 삭제:

```powershell
npx.cmd -y @coaspe/sap-abap-mcp@0.1.1 profile remove DEV100
```
...
```powershell
npm install -g @coaspe/sap-abap-mcp@0.1.1
```
...
- 프로필의 `allowedPackages`로 향후 쓰기 가능 패키지를 제한한다.
- `production` 프로필은 쓰기 도구를 기본 거부하고 별도 승인을 요구한다.
- 모든 쓰기 도구는 대상 `connectionId`와 Transport Request를 명시적으로 요구해야 한다.
...
```powershell
npx.cmd -y @coaspe/sap-abap-mcp@0.1.1 profile remove DEV100
npx.cmd -y @coaspe/sap-abap-mcp@0.1.1 profile remove QAS200
npx.cmd -y @coaspe/sap-abap-mcp@0.1.1 profile remove PRD100
```
