...
- 옵션 없는 `serve`: callable v1 도구 113개와 Resource 7개
- `--api-version v0`: 하위 호환용 기존 53개 도구
- `--api-version all`: v0 53개와 v1 113개를 함께 노출하는
...
```powershell
codex mcp remove sap-abap-b4d-local
codex mcp add sap-abap-b4d-local -- node "C:\src\sap-abap-mcp-v1\dist\src\index.js" serve --profile B4D --api-version all
codex mcp list
```

이 모드는 이름 inventory 확인에만 사용한다. 현재 기대값은 v0 53개와 v1
113개, 합계 166개다. mutation은 더 단순한 단일 API 전용 세션에서 수행한다.
...
그리고 smoke의 `113 v1 tools, 7 Resources`다. smoke가 실패하면 Claude
...
113개 전 행과 엄격한 RUN_OWNED `$TMP` mutation까지 한 세션에서 검증하려면
[`live-sap-v1-113-tool-tmp-test-prompt.ko.md`](live-sap-v1-113-tool-tmp-test-prompt.ko.md)의
...
- 연결 모드는 옵션 없는 `serve --profile B4D`, 즉 v1/all 기본값이다.
...
먼저 MCP discovery를 실제로 확인하라. `docs/v1-parity-matrix.md`의 113개
v1 target과 실제 광고 이름을 exact set으로 비교한다. toolset별 기대 수는
core 20, write 23, analysis 29, debug 10, operations 24, artifacts 7이다.
v0 이름이 하나라도 있거나 총수가 113개보다 많거나 적으면 live call을
...
3. expected 113/advertised/missing/extra/duplicate tool 수와 toolset별 수
