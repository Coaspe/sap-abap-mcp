...
const DOCUMENTATION = [
  ...
  "- Search results merge overlapping context in the primary response; compact-v1 summaries preserve oversized exact results by ID.",
  "- Object-centric responses reuse compact name/type identities; ATC findings use one object catalog instead of repeating metadata per finding.",
  "- Dump, trace, heartbeat, and capability details are summarized by default and exposed in full only through explicit detail actions or options.",
  "",
  "## Tool groups",
  "",
...
const SETTINGS = [
  ...
  "- authType: basic (default) or oauth_client_credentials (explicit opt-in).",
  "- username: SAP user name; required for Basic Auth and optional for OAuth client credentials.",
  "- tokenUrl, clientId, scope: non-secret OAuth client-credentials metadata. tokenUrl and clientId are required for OAuth.",
  ...
  "- Passwords and OAuth client secrets are stored separately in macOS Keychain or Windows DPAPI-encrypted files. Access tokens remain in process memory.",
  "",
  "## CLI",
  "",
  "setup",
  ...
  "profile add <id> --url <url> --client <nnn> [--language EN] [--environment development|quality|production] [--username <user>] [--packages ZPKG1,ZPKG2] [--auth-type basic|oauth-client-credentials] [--token-url <url> --client-id <id> [--scope <scope>]] [--login [--password-stdin]]",
  "profile list",
  "profile remove <id>",
